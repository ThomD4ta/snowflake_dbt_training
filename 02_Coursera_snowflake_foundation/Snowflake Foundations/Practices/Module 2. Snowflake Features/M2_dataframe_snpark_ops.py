...
def main(session: snowpark.Session): 
    ...
    df_table = session.table("DATAEXPERT_STUDENT.RAW_POS.MENU")
...
# load data using a query through session.sql instead of through session.table
    #df_table2 = session.sql("SELECT * FROM DATAEXPERT_STUDENT.RAW_POS.MENU LIMIT 5")
