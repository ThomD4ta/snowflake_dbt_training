-- User Define Functions (UDFs) and User Define Table Functions (UDTFs)
...
CREATE FUNCTION max_menu_price_converted(USD_to_new NUMBER)
  ...
  AS
  $$
    SELECT USD_TO_NEW * MAX(SALE_PRICE_USD) 
    FROM DATAEXPERT_STUDENT.RAW_POS.MENU
  $$
  ;
...
---> create a user-defined table function
CREATE FUNCTION menu_prices_above(price_ceiling NUMBER)
  RETURNS TABLE (item VARCHAR, price NUMBER)
  AS
  $$
    SELECT MENU_ITEM_NAME, SALE_PRICE_USD 
